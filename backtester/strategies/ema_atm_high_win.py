"""EMA 9/20 ATM Limit Scalper - high win-rate variant.
...
- No new orders or pending fills after the entry cutoff.

Tuning note: this variant was chosen from target>=2 sweeps for high win rate
with positive P&L. Its stop is still wide, so losses can be large.
"""
...
class Strategy:
    def setup(self, ctx):
        ...
        ctx.params["ema_gap"] = 6
        ctx.params["strike_offset"] = 3
        ctx.params["target_pts"] = 5.0
        ctx.params["sl_pts"] = 20
        ctx.params["cooldown_mins"] = 15
        ctx.params["trading_capital"] = 100_000
        ctx.params["chart_interval"] = "1min"

        # --- order / session hygiene ---------------------------------------
        ctx.params["order_ttl_bars"] = 9
        ctx.params["warmup_bars"] = 20
        ctx.params["session_open"] = (9, 15)
        ctx.params["entry_cutoff_hhmm"] = (15, 0)
    ...
    def on_bar(self, snapshot, ctx):
        ...
        if self._pending_limit is not None:
            ...
            if past_cutoff or square_off:
                ...
                self._clear_pending()
                tag = "after entry cutoff" if past_cutoff else "square-off"
        ...
        if past_cutoff:
            return ctx.skip(reason="after entry cutoff - no new orders")
        if square_off:
